...
def dipole_field(dip_r, dip_m, pos_r):
    ...
    # TODO: pos_r and dip_r cannot have both simultaneously Nx3 elements
    # If we have N dipoles we pass 1 ref point
    # If we pass N reference points we pass 1 dipole (but the result is different)
    r = pos_r - dip_r
    x, y, z = r[:, 0], r[:, 1], r[:, 2]
...
# TODO: can be defined as static method
def dipole_Bz(dip_r, dip_m, pos_r):
    """
    ...
    # Only return Bz
    return f * z + g * dip_m[:, 2]


def quadrupole_Bz(quad_r, quad_m, pos_r):
    """
...
def octupole_Bz(oct_r, oct_m, pos_r):
    """
    NOTE: THIS FUNCTION NEEDS TO BE REDEFINED (IT DEPENDS ON THE BASIS WE ARE
    EXPRESSING THE MULTIPOLE EXPANSION)
...
class MagneticSample(object):
    ...
    def generate_measurement_mesh(self):
        ...
        for j, y in enumerate(self.Sy_range):
            for i, x in enumerate(self.Sx_range):

                # Compute the z-component dipole field from all the N_particles
                # dipoles, at the (x,y) grid point
                field_contrib = dipole_Bz(self.dipole_positions,
                                          self.dipole_moments,
                                          [x, y, self.Hz])

                if isinstance(self.quadrupole_moments, np.ndarray):
                    field_contrib += quadrupole_Bz(self.dipole_positions,
                                                   self.quadrupole_moments,
                                                   [x, y, self.Hz]
                                                   )

                if isinstance(self.octupole_moments, np.ndarray):
                    field_contrib += octupole_Bz(self.dipole_positions,
                                                 self.octupole_moments,
                                                 [x, y, self.Hz]
                                                 )

                # Alternatively:
                # dipole_contrib = dipole_field(self.dipole_positions,
                #                               self.dipole_moments,
                #                               [x, y, self.Hz])[:, 2]

                # Sum their contribution to the (x,y) point
                Bz_grid[j, i] += np.sum(field_contrib)
