...
def fw_model_fun(sensor_dx=1e-6, sensor_dy=1e-6, overwrite=False, SAVEDIR="TEST_TMP"):
    """ """
    ...
    dipole_positions = np.array([[sample.Lx * 0.5, sample.Ly * 0.5, -sample.Lz * 0.05]])
...
def test_inversion_single_dipole_pylops(limit):
    ...
    for i in range(3):
        rel_diff = abs(
            inv_model.inv_multipole_moments[0][i] - expected_magnetization[i]
        )
