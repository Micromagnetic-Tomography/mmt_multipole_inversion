...
print('\n** Magnetization using area sensors')
...
# mag_area1_ums = np.sqrt(np.sum(mag_area1_ums[:, :3] ** 2, axis=1))
# 
# print('\n** Magnetization using point source sensors')
# for i, m in enumerate(mag_area1_ums):
#     print(f'i = {i + 1}   '
#           f'M_i = {m:>11.4f} A/m   '
#           f'M_dGroot = {dG_Area1UMS[i]:>11.4f} A/m   '
...
invPylops_area1_ums = minv.MultipoleInversion(BASE_DIR / "AREA1_UMS_METADICT.json",
                                              BASE_DIR / 'Area1_UMS_NPZ_ARRAYS.npz',
                                              expansion_limit='quadrupole',
                                              sus_functions_module='spherical_harmonics_basis'
                                              )

invPylops_area1_ums.Bz_array = np.loadtxt(data_dir / 'Area1-90-fig2MMT.txt')
invPylops_area1_ums.sensor_dims = ()
invPylops_area1_ums.compute_inversion(method="pylops")

magPylops_area1_ums = invPylops_area1_ums.inv_multipole_moments / invPylops_area1_ums.volumes[:, None]
magPylops_area1_ums = np.sqrt(np.sum(magPylops_area1_ums[:, :3] ** 2, axis=1))
# invPylops_area1_ums = print(invPylops_area1_ums)

print('\n** Magnetization using point source sensors')
for i, m in enumerate(magPylops_area1_ums):
    print(f'i = {i + 1}   '
          f'M_i = {m:>11.4f} A/m   '
          f'M_dGroot = {dG_Area1UMS[i]:>11.4f} A/m   '
          f'Volume: {invPylops_area1_ums.volumes[i] * 1e18:>9.4f} µm^3',
          )
print()

# invTorch_area1_ums = minv.MultipoleInversion(BASE_DIR / "AREA1_UMS_METADICT.json",
#                                              BASE_DIR / 'Area1_UMS_NPZ_ARRAYS.npz',
#                                              expansion_limit='quadrupole',
#                                              sus_functions_module='spherical_harmonics_basis'
#                                              )
# 
# invTorch_area1_ums.Bz_array = np.loadtxt(data_dir / 'Area1-90-fig2MMT.txt')
# invTorch_area1_ums.sensor_dims = ()
# invTorch_area1_ums.compute_inversion(method="pylops")
# 
# magTorch_area1_ums = inv_area1_ums.inv_multipole_moments / inv_area1_ums.volumes[:, None]
# # magTorch_area1_ums = np.sqrt(np.sum(mag_area1_ums[:, :3] ** 2, axis=1))
# magTorch_area1_ums = print(mag_area1_ums)
# 
# print('\n** Magnetization using point source sensors')
# for i, m in enumerate(magTorch_area1_ums):
#     print(f'i = {i + 1}   '
#           f'M_i = {m:>11.4f} A/m   '
#           f'M_dGroot = {dG_Area1UMS[i]:>11.4f} A/m   '
#           f'Volume: {invTorch_area1_ums.volumes[i] * 1e18:>9.4f} µm^3',
#           '\n')
