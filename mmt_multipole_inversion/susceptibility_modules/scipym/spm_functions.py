...
def dipole_Bz_sus(xin, N_sensors, N_particles, dip_r, pos_r, yout, n_col_stride):
    for i in range(N_sensors):
        ...
        #  Only return Bz
        yout[i] += np.dot(f * (3 * x * z), xin[::n_col_stride])
        yout[i] += np.dot(f * (3 * y * z), xin[1::n_col_stride])
        yout[i] += np.dot(f * (3 * z2 - r2), xin[2::n_col_stride])
...
def quadrupole_Bz_sus(xin, N_sensors, N_particles, dip_r, pos_r, yout, n_col_stride):
    for i in range(N_sensors):
        ...
        yout[i] += np.dot(g * np.sqrt(3 / 2) * z * (-3 * r2 + 5 * z2), xin[3::n_col_stride])
        yout[i] += np.dot(g * -np.sqrt(2) * x * (r2 - 5 * z2), xin[4::n_col_stride])
        yout[i] += np.dot(g * -np.sqrt(2) * y * (r2 - 5 * z2), xin[5::n_col_stride])
        yout[i] += np.dot(g * (5 / np.sqrt(2)) * (x2 - y2) * z, xin[6::n_col_stride])
        yout[i] += np.dot(g * 5 * np.sqrt(2) * x * y * z, xin[7::n_col_stride])
